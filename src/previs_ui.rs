...
use eframe::App;
use egui::{TextureHandle, Rect, Vec2, Pos2, Ui, Color32};
...
fn draw_screens(ui: &mut Ui, screens: &Screens){
    ...
    for (screen_info, texture_handle) in screens {
        ...
        let image_offset_pos = Pos2::new(screen_offset.x*image_scale, screen_offset.y*image_scale)
        ...
        let info_text = format!("universe {}\nchannel {}", 
            screen_info.mapping.address.universe, 
            screen_info.mapping.address.channel
        );
...
impl PrevisApp {
    ...
    pub fn draw(&mut self, ctx: &egui::Context){
        ...
        // egui::SidePanel::left("debug_textures").default_width(256.0).show(ctx, |ui| {
        //     ctx.texture_ui(ui);
        // });
